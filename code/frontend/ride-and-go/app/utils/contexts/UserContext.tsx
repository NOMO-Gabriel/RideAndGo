...
export const UserProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Charger les informations utilisateur depuis le localStorage au montage du composant
  useEffect(() => {
    const savedUser = localStorage.getItem('user');
    if (savedUser) setUser(JSON.parse(savedUser)); // Charger l'utilisateur si disponible
  }, []);
  const login = async (
    ...
    try {
      ...
      throw error; // Remonte l'erreur pour être capturée dans le formulaire
    }
  };
